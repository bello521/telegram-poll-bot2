...
def load_schedule():
    ...
    # Make sure this matches your actual file name (schedule.json vs test_schedule.json)
...
async def send_leaderboard(context):
    try:
        ...
        updated_matches = [
            int(m) for m, p in data["polls"].items() if p["updated"]
        ]

        latest_match = "0"
        if updated_matches:
            latest_match = str(max(updated_matches))
...
def main():
    ...
    if RENDER_EXTERNAL_HOSTNAME:
        # If running on Render, use Webhooks
        ...
        # If running locally on your PC, fallback to polling
